    ...
    def validate_and_fill_filename_templates(cls, values):
        ...
        for field in ["transactions", "passport_blacklist", "terminals"]:
            ...
            if "{date}" not in value:
                raise ValueError(f"{value} for {field} should contain {{date}} placeholder.")
        return values

    def generate_file_names(self, date: str, date_format: str = "%d%m%Y") -> Dict[str, str]:
        """
        ...
        try:
            ...
            raise ValueError(f"Invalid date format: {date}. Could not format with {date_format}.")
